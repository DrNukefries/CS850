...
-   **Submission**: Turn in as a **PDF** and the **source code** (qmd or ipynb) on [MyCourses](http://mycourses.unh.edu). You can directly submit an IPython ipynb notebook, or convert it to qmd (see <https://quarto.org/docs/tools/jupyter-lab.html#converting-notebooks>).
...
You are free to use Python, but then you are on your own when it comes to installing all the necessary tools. You can, however, use the Python version of the ISLR textbook. We cannot guarantee being able to help with the assignments or installation of Python development tools. You can use Rstudio, VS Code, vim, emacs, jupyter, or any editor that you may be familiar with.
